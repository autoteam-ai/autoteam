...
`aiwf init` 生成的 Makefile 里，三个目标都是会直接报错的 `AIWF-TODO` 桩，逼你把它们改成真实命令。这一步决定了后面所有自动化的上限：`make check` 不够严，编排得再好，也只是更快地把不合格的代码送进主干。
...
各技术栈的例子见 skill 里的 [adapt-make.md](../../skills/ai-workflow/references/adapt-make.md)。

## AGENTS.md

`aiwf init` 只在 AGENTS.md 末尾追加四条规则（受管块）。其余要你自己写，原则：
...
详见 [write-agents-md.md](../../skills/ai-workflow/references/write-agents-md.md)。
