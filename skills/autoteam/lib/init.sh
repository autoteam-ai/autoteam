...
装完在 .autoteam/.lock.json 记下每个文件的 sha256，升级用 autoteam upgrade。
指定文件时只处理这些文件，例如放弃本地改动、换成模板：autoteam init --force .autoteam/playbook.md
...
EOF
  ...
  # 已有 lock 时保留它的版本：init 不覆盖改过的旧文件，版本号该由 upgrade 来推进
  lock_save "${LOCK_VERSION:-$(autoteam_version)}"
  [ "$only" = " " ] || return 0
...
init_install() {
  ...
  case $mode in
    file|exec|config)
      current=$(cat "$target" 2>/dev/null; printf x)
      current=${current%x}
      if [ ! -e "$target" ]; then
        init_write "$target" "$content" "$mode"
        ok "新建 $target"
      elif [ "$current" = "$content" ]; then
        info "已是最新 $target"
  ...
  if lock_tracked "$mode"; then lock_record "$target" "$mode" "$(lock_sha_want "$mode" "$content")"; fi
}

# 打印已安装文件和模板渲染结果的差异，有差异返回 1。参数：<目标> <方式> <渲染内容> [模板一侧的标签]
init_diff_one() {
  local target=$1 mode=$2 content=$3 label=${4:-模板} tmp
  tmp=$(autoteam_tmpdir)
  case $mode in
    block)
      printf '%s' "${content%$'\n'}" > "$tmp/want"
      printf '%s' "$(block_extract "$target")" > "$tmp/have"
      diff -u --label "$target（受管块）" --label "$label" "$tmp/have" "$tmp/want"
      ;;
    *)
      printf '%s' "$content" > "$tmp/want"
      diff -u --label "$target" --label "$label" "$target" "$tmp/want"
      ;;
  esac
}
...
选项：
  --check   有差异时退出码为 1，给 CI 用来挡住模板漂移。跳过 autoteam.conf、
            registry.yaml 这类用户数据；.lock.json 显示本地改过的文件报告为"本地已修改"，不算漂移
EOF
}
...
cmd_diff() {
  ...
  local root tpl target mode content changed=0
  root=$(repo_root)
  cd "$root" || die "进不去 $root"
  ...
  while read -r tpl target mode; do
    ...
    if [ "$check" = 1 ]; then
      ...
      if lock_modified "$target" "$mode"; then info "本地已修改 $target（和 .lock.json 的记录不同，不算漂移）"; continue; fi
    fi
    ...
    case $mode in
      makefile) [ "$only" = " " ] || init_diff_one "$target" "$mode" "$content" || true ;;
      *) init_diff_one "$target" "$mode" "$content" || changed=1 ;;
    esac
  done <<EOF
$(autoteam_manifest)
root/gitignore.block                  .gitignore                              block
EOF
  if [ "$changed" = 0 ]; then
    ...
    fail "有文件和模板不一致"
    hint "模板更新了、文件还是装机时的样子：运行 autoteam upgrade"
    return 1
  fi
  return 0
}
