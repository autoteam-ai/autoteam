...
# aiwf github：仓库设置、规则集、environment、机器账号。默认只预览。

github_usage() {
  cat <<'EOF'
用法：aiwf github [选项]

按 ops/agents/aiwf.conf 配置 GitHub 仓库。默认只预览，加 --apply 才执行。

  --apply                  执行改动
  --trial                  单账号试用模式：规则集不要求审批（写代码和评审用同一个 GitHub 账号时用）
  --bots impl=<用户>,review=<用户>,planner=<用户>
                           邀请机器账号为协作者（也可以写在 aiwf.conf 的 AIWF_*_BOT）
  --repo <owner/name>      覆盖 aiwf.conf 里的仓库

会做的事：
  1. 仓库设置：允许自动合并、合并后删分支、只保留 squash 合并
  2. 规则集 ai-workflow（默认分支）：禁止删除和强推、必须走 PR、1 个审批、
     新提交作废旧审批、规则文件要 Code Owner 审批、最后一次推送要别人批准、
     必需检查 check（只认 GitHub Actions 上报）、组织仓库再加合并队列
  3. 部署 environment（aiwf.conf 的 AIWF_DEPLOY_ENVIRONMENT）
  4. 邀请机器账号，打印每个账号的 token 权限
EOF
}

GH_ACTIONS_APP_ID=15368
AIWF_RULESET_NAME=ai-workflow
...
cmd_github() {
  ...
  conf_load "$root"
  [ -n "$AIWF_REPO" ] || die "aiwf.conf 里没有 AIWF_REPO，先运行 aiwf init"
  gh auth status >/dev/null 2>&1 || die "gh 还没登录：gh auth login"

  section "仓库 $AIWF_REPO"
  local repo_json owner_type visibility level rulesets_ok=1
  gh_call GET "repos/$AIWF_REPO" || die "读不到仓库 $AIWF_REPO：$GH_OUT"
  repo_json=$GH_OUT
  owner_type=$(jq -r '.owner.type' <<<"$repo_json")
  visibility=$(jq -r '.visibility' <<<"$repo_json")
  [ "$(jq -r '.permissions.admin' <<<"$repo_json")" = true ] || die "需要 $AIWF_REPO 的管理员权限"

  if ! gh_call GET "repos/$AIWF_REPO/rulesets"; then
    case $GH_OUT in
...
github_settings() {
  ...
  planned "PATCH repos/$AIWF_REPO $diff"
  [ "$AIWF_APPLY" = 1 ] || return 0
  if ! gh_call PATCH "repos/$AIWF_REPO" "$diff"; then
    # 有的套餐不允许打开自动合并，去掉这一项再试
    diff=$(jq -c 'del(.allow_auto_merge)' <<<"$diff")
    gh_call PATCH "repos/$AIWF_REPO" "$diff" || { fail "修改仓库设置失败：$GH_OUT"; return 0; }
  fi
  gh_call GET "repos/$AIWF_REPO" || { fail "回读仓库设置失败：$GH_OUT"; return 0; }
  local missed
...
github_ruleset_json() {
  ...
  jq -n --argjson approvals "$approvals" --argjson code_owner "$code_owner" \
    ...
    --arg check "$AIWF_CHECK_NAME" --arg name "$AIWF_RULESET_NAME" --argjson app "$GH_ACTIONS_APP_ID" '
    {
      name: $name,
      target: "branch",
      enforcement: "active",
...
github_ruleset() {
  ...
  gh_call GET "repos/$AIWF_REPO/rulesets" || { fail "读取规则集失败：$GH_OUT"; return 0; }
  id=$(jq -r --arg n "$AIWF_RULESET_NAME" '.[] | select(.name == $n) | .id' <<<"$GH_OUT" | head -n1)

  if [ -n "$id" ]; then
    gh_call GET "repos/$AIWF_REPO/rulesets/$id" || { fail "读取规则集 $id 失败：$GH_OUT"; return 0; }
    if github_ruleset_matches "$GH_OUT" "$want"; then
      ok "规则集已符合（id $id）"
      return 0
    fi
    planned "PUT repos/$AIWF_REPO/rulesets/$id（更新为 $(github_ruleset_summary "$want")）"
    [ "$AIWF_APPLY" = 1 ] || return 0
    github_ruleset_write PUT "repos/$AIWF_REPO/rulesets/$id" "$want"
  else
    planned "POST repos/$AIWF_REPO/rulesets（$(github_ruleset_summary "$want")）"
    [ "$AIWF_APPLY" = 1 ] || return 0
    github_ruleset_write POST "repos/$AIWF_REPO/rulesets" "$want"
  fi
}
...
github_environment() {
  local level=$1 env=$AIWF_DEPLOY_ENVIRONMENT
  if [ -z "$env" ]; then
    info "aiwf.conf 没配置 AIWF_DEPLOY_ENVIRONMENT，deploy.yml 不使用 environment"
    return 0
  fi
  if gh_call GET "repos/$AIWF_REPO/environments/$env"; then
    ok "environment $env 已存在"
    return 0
  fi
  planned "PUT repos/$AIWF_REPO/environments/$env"
  [ "$AIWF_APPLY" = 1 ] || return 0
  if gh_call PUT "repos/$AIWF_REPO/environments/$env" '{}'; then
    ok "已创建 environment $env"
  else
    warn "创建 environment 失败：$GH_OUT"
    hint "GitHub Free 的私有仓库不支持 environment：把 aiwf.conf 的 AIWF_DEPLOY_ENVIRONMENT 置空，并删掉 deploy.yml、rollback.yml 里的 environment 行"
  fi
}

github_bots() {
  local bots=$1 owner_type=$2 pair role user any=0
  local impl=$AIWF_IMPL_BOT review=$AIWF_REVIEW_BOT planner=$AIWF_PLANNER_BOT
  if [ -n "$bots" ]; then
  ...
  if [ "$any" = 0 ]; then
    ...
    hint "准备好账号后写进 aiwf.conf 的 AIWF_IMPL_BOT / AIWF_REVIEW_BOT / AIWF_PLANNER_BOT，或用 --bots 传入"
    return 0
  fi
...
github_invite() {
  local role=$1 user=$2
  if gh_call GET "repos/$AIWF_REPO/collaborators/$user"; then
    ok "$role 账号 $user 已是协作者"
    return 0
  fi
  if gh_call GET "repos/$AIWF_REPO/invitations" \
    && jq -e --arg u "$user" '.[] | select(.invitee.login == $u)' <<<"$GH_OUT" >/dev/null; then
    warn "$role 账号 $user 的邀请还没接受：用该账号登录 GitHub 接受邀请"
    return 0
  fi
  planned "PUT repos/$AIWF_REPO/collaborators/$user（permission=push）"
  [ "$AIWF_APPLY" = 1 ] || return 0
  if gh_call PUT "repos/$AIWF_REPO/collaborators/$user" '{"permission":"push"}'; then
...
github_extra_checks() {
  if gh_call GET "repos/$AIWF_REPO/codeowners/errors"; then
    local n
    ...
    info "默认分支上还没有 CODEOWNERS（合并 aiwf init 生成的文件后再检查）"
  fi
  if gh secret list --repo "$AIWF_REPO" --json name --jq '.[].name' 2>/dev/null | grep -qx MULTICA_DEPLOY_HOOK; then
    ok "secret MULTICA_DEPLOY_HOOK 已设置"
  else
    info "secret MULTICA_DEPLOY_HOOK 还没设置，aiwf multica --apply 会创建部署 webhook 并写入"
  fi
}
