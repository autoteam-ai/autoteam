...
# aiwf multica / aiwf runtimes：自定义状态、agent、项目、autopilot、部署 webhook。默认只预览。
...
按 ops/agents/ 下的文件配置 Multica 工作区。默认只预览，加 --apply 才执行。
  ...
  --profile <名字>        multica CLI 的 profile（默认：已配置的默认 profile，
                          否则 ~/.multica/profiles 下唯一的 profile；也可用 AIWF_MULTICA_PROFILE）
  --workspace <slug|ID>   工作区（默认 aiwf.conf 的 AIWF_MULTICA_WORKSPACE）
...
会做的事：
  ...
  3. 项目（AIWF_MULTICA_PROJECT），挂上 GitHub 仓库资源
...
mc_resolve_bin() {
  [ -n "${MC_BIN:-}" ] && return 0
  if [ -n "${AIWF_MULTICA_BIN:-}" ]; then
    MC_BIN=$AIWF_MULTICA_BIN
...
# profile：参数 > AIWF_MULTICA_PROFILE > 已配置的默认 profile > ~/.multica/profiles 下唯一的 profile
mc_resolve_profile() {
  MC_PROFILE=${1:-${AIWF_MULTICA_PROFILE:-}}
  MC_ARGS=()
...
mc_resolve_workspace() {
  ...
  [ -n "$ws" ] || die "没有指定 Multica 工作区：在 aiwf.conf 写 AIWF_MULTICA_WORKSPACE，或用 --workspace"
...
cmd_multica() {
  ...
  section "连接 Multica"
  multica_setup "$profile" "${ws:-$AIWF_MULTICA_WORKSPACE}"
...
multica_statuses() {
  ...
  while IFS=$'\t' read -r key name category color icon desc; do
    ...
    [ "$AIWF_APPLY" = 1 ] || continue
    body=$(jq -nc --arg key "$key" --arg name "$name" --arg category "$category" \
      --arg color "$color" --arg icon "$icon" --arg desc "$desc（ai-workflow）" \
...
runtimes_usage() {
  cat <<'EOF'
用法：aiwf runtimes [--profile <名字>] [--workspace <slug>]
...
multica_agent_args() {
  ...
  MC_AGENT_ARGS=(--runtime-id "$rid" --instructions "$(read_file "ops/agents/$role.md")"
    --description "ai-workflow 的 $role（由 aiwf 管理，指令源文件 ops/agents/$role.md）")
...
multica_agent_create() {
  ...
  if [ "$AIWF_AGENT_ACCESS" = workspace ]; then
...
multica_project() {
  local title=${AIWF_MULTICA_PROJECT:-${AIWF_REPO##*/}} url="https://github.com/$AIWF_REPO" projects out res
  ...
  if [ -z "$MC_PROJECT_ID" ]; then
    ...
    [ "$AIWF_APPLY" = 1 ] || return 0
    if out=$(mc project create --title "$title" --repo "$url" --description "ai-workflow 管理的项目，仓库 $AIWF_REPO" --output json 2>&1); then
...
multica_autopilot() {
  ...
  if [ "$mode" = create_issue ]; then
    ...
    if [ "$subscriber" = human ]; then
      subscriber=${AIWF_HUMAN:-$(mc user profile get --output json 2>/dev/null | jq -r '.name // empty')}
    fi
...
multica_schedule_trigger() {
  ...
  if [ -z "$tid" ]; then
    planned "给「$title」加定时触发 $cron（$AIWF_TIMEZONE）"
    [ "$AIWF_APPLY" = 1 ] || return 0
    if out=$(mc autopilot trigger-add "$id" --kind schedule --cron "$cron" --timezone "$AIWF_TIMEZONE" --output json 2>&1); then
      ok "已加定时触发 $cron"
    else
      fail "加定时触发失败：$out"
    fi
  ...
  if [ "$have_cron" = "$cron" ] && [ "$have_tz" = "$AIWF_TIMEZONE" ]; then
    ok "定时触发已是 $cron（$AIWF_TIMEZONE）"
    return 0
  fi
  planned "把「$title」的定时触发从 ${have_cron:-?}（${have_tz:-?}）改为 $cron（$AIWF_TIMEZONE）"
  [ "$AIWF_APPLY" = 1 ] || return 0
  if out=$(mc autopilot trigger-update "$id" "$tid" --cron "$cron" --timezone "$AIWF_TIMEZONE" --output json 2>&1); then
    ok "已更新定时触发"
  else
    fail "更新定时触发失败：$out"
  fi
...
multica_webhook_trigger() {
  ...
  if [ -z "$url" ]; then
    fail "拿不到 webhook 地址：在 Multica 界面的 autopilot「$title」里复制地址，执行 gh secret set MULTICA_DEPLOY_HOOK --repo $AIWF_REPO"
    return 0
  fi
  if printf '%s' "$url" | gh secret set MULTICA_DEPLOY_HOOK --repo "$AIWF_REPO" >/dev/null 2>&1; then
