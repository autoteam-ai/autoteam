...
只读检查本地文件、GitHub、Multica 是否按 ai-workflow 配好。有 ❌ 时退出码为 1。

  --skip-github            不检查 GitHub
  --skip-multica           不检查 Multica
  --profile <名字>         multica profile
  --workspace <slug|ID>    Multica 工作区（默认 aiwf.conf 的 AIWF_MULTICA_WORKSPACE）
EOF
}
...
cmd_doctor() {
  ...
  if ! conf_exists "$root"; then
    fail "没有 $AIWF_CONF_REL：先运行 aiwf init"
    print_summary
    return 1
  fi
  conf_load "$root"
  ...
  if [ "$skip_gh" = 1 ]; then
    ...
    section "GitHub $AIWF_REPO"
    doctor_github
  fi

  if [ "$skip_mc" = 1 ]; then
    info "跳过 Multica 检查"
  elif [ -z "${ws:-$AIWF_MULTICA_WORKSPACE}" ] && [ -z "${MULTICA_WORKSPACE_ID:-}" ]; then
    section "Multica"
    warn "aiwf.conf 没有 AIWF_MULTICA_WORKSPACE，跳过 Multica 检查"
  else
    section "Multica"
    # 在子 shell 里跑：multica 出错时 die 只结束这一段；计数通过文件带回来
    local counts e w
    counts=$(aiwf_tmpdir)/doctor-multica
    (
      AIWF_ERRORS=0 AIWF_WARNINGS=0
      doctor_multica "$profile" "${ws:-$AIWF_MULTICA_WORKSPACE}" "$rows"
      printf '%s %s\n' "$AIWF_ERRORS" "$AIWF_WARNINGS" > "$counts"
    ) || true
    if [ -f "$counts" ]; then
      read -r e w < "$counts"
      AIWF_ERRORS=$((AIWF_ERRORS + e)) AIWF_WARNINGS=$((AIWF_WARNINGS + w))
    else
      AIWF_ERRORS=$((AIWF_ERRORS + 1))
    fi
  fi
  ...
  while read -r tpl target mode; do
    ...
    case $mode in
      ...
      block)
        ...
        grep -qxF "$start" "$target" || fail "$target 里没有 ai-workflow 受管块：运行 aiwf init"
        ;;
    esac
  done <<EOF
$(aiwf_manifest)
EOF
  if [ -n "$missing" ]; then
    fail "缺少文件：$missing（运行 aiwf init）"
  else
    ok "工作流文件齐全"
  fi
  ...
  if [ -f Makefile ]; then
    ...
    if grep -q 'AIWF-TODO' Makefile; then
      fail "Makefile 里还有 AIWF-TODO 桩：把 check / dev / deploy 改成真实命令"
    else
...
doctor_github() {
  ...
  if ! gh_call GET "repos/$AIWF_REPO"; then
    fail "读不到仓库：$GH_OUT"
    return 0
  fi
  ...
  if [ "$(jq -r '.allow_squash_merge and (.allow_merge_commit | not) and (.allow_rebase_merge | not)' <<<"$repo")" = true ]; then
    ...
    warn "合并方式不止 squash：aiwf github --apply"
  fi
  ...
  if ! gh_call GET "repos/$AIWF_REPO/rulesets"; then
    case $GH_OUT in
      *"Upgrade to GitHub Pro"*)
        ...
        hint "仓库改为公开、升级 GitHub Pro 或迁到 Team 组织后，运行 aiwf github --apply" ;;
      *) fail "读取规则集失败：$GH_OUT" ;;
    esac
  else
    id=$(jq -r --arg n "$AIWF_RULESET_NAME" '.[] | select(.name == $n) | .id' <<<"$GH_OUT" | head -n1)
    if [ -z "$id" ]; then
      fail "没有规则集 $AIWF_RULESET_NAME：aiwf github --apply"
    elif gh_call GET "repos/$AIWF_REPO/rulesets/$id"; then
      if [ "$(jq -r '.enforcement' <<<"$GH_OUT")" != active ]; then
        fail "规则集 $AIWF_RULESET_NAME 没有启用（enforcement=$(jq -r '.enforcement' <<<"$GH_OUT")）"
      else
  ...
  if gh secret list --repo "$AIWF_REPO" --json name --jq '.[].name' 2>/dev/null | grep -qx MULTICA_DEPLOY_HOOK; then
    ...
    fail "没有 secret MULTICA_DEPLOY_HOOK：部署结果通知不到 Planner（aiwf multica --apply）"
  fi
  if gh_call GET "repos/$AIWF_REPO/codeowners/errors"; then
    n=$(jq '.errors | length' <<<"$GH_OUT")
    if [ "$n" = 0 ]; then ok "CODEOWNERS 没有错误"; else fail "CODEOWNERS 有 $n 个错误（aiwf github 查看详情）"; fi
  else
    warn "默认分支上还没有 CODEOWNERS"
  fi
  local role user
  for role in IMPL REVIEW PLANNER; do
    eval "user=\${AIWF_${role}_BOT:-}"
    [ -n "$user" ] || continue
    if gh_call GET "repos/$AIWF_REPO/collaborators/$user"; then ok "机器账号 $user 已是协作者"; else fail "机器账号 $user 还不是协作者"; fi
  done
  if [ -z "$AIWF_IMPL_BOT$AIWF_REVIEW_BOT" ]; then
    warn "没有配置机器账号：Implementer 和 Reviewer 用同一个 GitHub 账号时，GitHub 不允许批准自己的 PR"
  fi
  local run
  run=$(gh run list --repo "$AIWF_REPO" --workflow gate.yml --limit 1 --json conclusion,status,headBranch,url 2>/dev/null | jq -c '.[0] // empty')
  if [ -z "$run" ]; then
...
$(aiwf_statuses)
EOF
    if [ -z "$missing" ]; then ok "自定义状态齐全：approved、code_review、rework、shipping"; else fail "缺少自定义状态：$missing（aiwf multica --apply）"; fi
  else
  ...
  if [ -n "$rows" ]; then
    while IFS=$'\t' read -r name role _; do
      ...
      if [ -z "$id" ]; then fail "agent $name 不存在（aiwf multica --apply）"; continue; fi
      ...
      if [ "$(jq -r '.instructions' <<<"$cur")" != "${want%$'\n'}" ] && [ "$(jq -r '.instructions' <<<"$cur")" != "$want" ]; then
        fail "agent $name 的指令和 ops/agents/$role.md 不一致（指令漂移）：aiwf multica --apply"
...
EOF
  ...
  project=$(mc project list --output json | jq -r --arg t "${AIWF_MULTICA_PROJECT:-${AIWF_REPO##*/}}" '[.[] | select(.title == $t)][0].id // empty')
  if [ -n "$project" ]; then ok "项目 ${AIWF_MULTICA_PROJECT:-${AIWF_REPO##*/}} 存在"; else fail "项目 ${AIWF_MULTICA_PROJECT:-${AIWF_REPO##*/}} 不存在（aiwf multica --apply）"; fi
  ...
  for f in ops/agents/autopilots/*.md; do
    ...
    if [ -z "$cur" ]; then fail "autopilot「$title」不存在（aiwf multica --apply）"; continue; fi
    id=$(jq -r '.id' <<<"$cur")
    local ntrig status
