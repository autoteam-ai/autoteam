...
- **「部署结果」autopilot 会查 Multica 配置漂移**。规则文件合并之后，Multica 上的 agent 指令不会自动更新，原来只靠人记得跑 `autoteam multica --apply`——漂移是静默的。现在每次部署后 Planner 会跑一次预览，有没同步的就贴进运营笔记并提及人。**这是当前唯一一个「只能由人做、平台也不拦」的操作**，记在了闸门清单里。
