...
- `skills/autoteam/` 是唯一的实现，必须自包含：`npx skills add` 只会拷贝这个目录。
  ...
  - `templates/` 装进用户项目的文件（`root/` 装进仓库根、`autoteam/` 装进 `ops/agents/`），占位符写 `{{AUTOTEAM_名字}}`；
...
- `package.json` 和 `scripts/release.sh` 是 npm 包（包名 `autoteam`）的清单和发布脚本；`files` 只放 `skills/autoteam/`、`docs/`，新增顶层目录要同步它。
