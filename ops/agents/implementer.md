...
4. 提 PR 后，读取 `ops/agents/autoteam.conf` 的 `AUTOTEAM_CODEOWNERS_GATE`（未设置按 `on`）：
...
- 大范围重构：因为改动会没法评审；
- 没有任务要求时顺手修改 `.github/`、`ops/agents/`、`Makefile`、`.jscpd.json`：这些是约束 agent 的规则文件。任务明确要求改它们时可以直接改并提 PR，由 CODEOWNERS 要求的人工批准把关，不必另行等人授权；没有任务要求，发现问题只写进评论的“范围外发现”。
